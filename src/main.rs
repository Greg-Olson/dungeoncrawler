...
impl State {
    ...
    fn game_over(&mut self, ctx: &mut BTerm) {
        ctx.set_active_console(2);// <callout id="co.losing.setlayer" />
        ctx.print_color_centered(2, RED, BLACK, "Your quest has ended."); // <callout id="co.losing.printing" />
        ...
        if let Some(VirtualKeyCode::Key1) = ctx.key {// <callout id="co.losing.ifkey" />
            self.ecs = World::default();// <callout id="co.losing.newuniverse" />
            ...
            self.resources.insert(TurnState::AwaitingInput);// <callout id="co.losing.turn_state" />
        }
    }
    //END: game_over_man
}
